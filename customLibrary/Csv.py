...
def update_field_with_value(file_name, field_name, ind, data):
    df = pd.read_csv(file_name)
    df.at[ind, field_name] = data
    df.to_csv(file_name, index=False)
...
def clear_column_entries_excel_file(filename, sheet_name, col_name):
    mylist = []
    excel_data_df = pandas.read_excel(filename, sheet_name=sheet_name)
    rows = len(excel_data_df.index)
    print(rows)
    for elements in range(rows):
        ...
        mylist.append(elements)
    print(mylist)
    excel_data_df1 = excel_data_df.drop(index=mylist, columns=col_name)
    excel_data_df1.to_excel(filename, sheet_name)
